...
- (id)initWithFrame:(CGRect)frame andValue:(float)newValue andMinValue:(int)minValue{
    if ((self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, SLIDER_HEIGHT)])) {
        ...
        [self moveSliderTo018];
        
        [self moveMaxSliderTo:100-newValue andMinValue:minValue];
...
- (void)setInRangeTrackImage:(UIImage *)image {
	trackImageView.frame = CGRectMake(10,trackImageView.frame.origin.y, 280, trackImageView.frame.size.height);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    NSLog(@"Min Slider %f",minSlider.frame.origin.x);
    if(minSlider.frame.origin.x<65.f || maxSlider.frame.origin.x<65.f)
        return;
}
-(void)moveSliderTo018
{
    float newX = MAX(
                     0,
                     MIN(
                         minSlider.frame.origin.x+47,
...
-(void)moveMinSliderTo18
{

    minSlider.frame = CGRectMake(min*self.frame.size.width, (SLIDER_HEIGHT-self.frame.size.height)/2.0, self.frame.size.height, self.frame.size.height);
    

}
-(void)moveMaxSliderTo:(float)newValue andMinValue:(int)minValue
{
    NSLog(@"%f",maxSlider.frame.origin.x);
    
        float newX = MAX(
                         0,
                         MIN(
                             minSlider.frame.origin.x+minValue,
                             self.frame.size.width-self.frame.size.height*2.0-minimumRangeLength*(self.frame.size.width-self.frame.size.height*2.0))
                         );
        ...
        maxSlider.frame = CGRectMake(
                                     MAX(
                                         maxSlider.frame.origin.x-newValue*2.8f,
                                         minSlider.frame.origin.x+self.frame.size.height+minimumRangeLength*(self.frame.size.width-self.frame.size.height*2.0)
                                         ),
                                     maxSlider.frame.origin.y,
                                     self.frame.size.height,
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	UITouch *touch = [touches anyObject];
    NSLog(@"%f",minSlider.frame.origin.x);
	float deltaX = [touch locationInView:self].x - [touch previousLocationInView:self].x;
    
	if (trackingSlider == minSlider && minSlider.frame.origin.x>=46.f) {
	...
	} else if (trackingSlider == maxSlider && maxSlider.frame.origin.x>=46.f) {
	...
	else if(trackingSlider == minSlider)
    {
        min = 0.17f;
        [self moveMinSliderTo18];
    }
