...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetFillColorWithColor(context, 
                                   [UIColor colorWithHexString:@"0x1a4d81"].CGColor);
    CGContextFillPath(context);
    ...
    CGContextSetFillColorWithColor(context, 
                                   [UIColor colorWithHexString:@"0x5CB8E6"].CGColor);
    for (int i =0; i<[weekdays count]; i++) {
        NSString *weekdayValue = [(NSString *)[weekdays objectAtIndex:i] uppercaseString];
        UIFont *font = [UIFont fontWithName:@"MyriadPro-Cond" size:12];
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xdff4ff"].CGColor);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (i<firstWeekDay) { //previous month
            ...
            CGRect circlePoint = CGRectMake(targetX+3, targetY+3, 37, 37);
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextSetRGBStrokeColor(context, 0, 0, 1.0, 1.0);
            CGRect circlePoint = CGRectMake(targetX+3, targetY+3, 37, 37);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xdddddd"].CGColor);
            CGContextFillEllipseInRect(context, circlePoint);
            
            UIImageView *bgView = [[UIImageView alloc] initWithFrame:CGRectMake(targetX+3, targetY+2, 37, 37)];
            [bgView setImage:[UIImage imageNamed:@"circle_interests"]];
            [self addSubview:bgView];
        }
        
        
        //[date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue-Normal" size:17] lineBreakMode:UILineBreakModeClip alignment:NSTextAlignmentCenter];
        
        //вкфц сшксду
        
        
        UILabel *lblDate = [[UILabel alloc] initWithFrame:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight)];
        lblDate.text = date;
        lblDate.textAlignment = NSTextAlignmentCenter;
        [lblDate setEnabled:YES];
        NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth) ? @"0xaaaaaa" : @"0x383838";
        lblDate.font = [UIFont fontWithName:@"MyriadPro-Cond" size:19];
        [lblDate setTextColor:[UIColor colorWithHexString:hex]];
        [lblDate drawTextInRect:CGRectMake(targetX, targetY, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight)];
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        UIImageView *bgView = [[UIImageView alloc] initWithFrame:CGRectMake(targetX-2, targetY-35, 37, 37)];
        [bgView setImage:[UIImage imageNamed:@"circle_interests"]];
        [self addSubview:bgView];
        
        
        UIImageView *eventView = [[UIImageView alloc] initWithFrame:CGRectMake(targetX-2, targetY-35, 35, 35)];
        [eventView setImage:(targetDate%2==0?[UIImage imageNamed:@"surfing_interests"]:[UIImage imageNamed:@"alpinizm_interests"])];
        eventView.layer.cornerRadius = 20;
        eventView.clipsToBounds = YES;
        [self addSubview:eventView];
        //Small date
        UILabel *lblSDate = [[UILabel alloc] initWithFrame:CGRectMake(targetX+20, targetY-40, 20, 20)];
        lblSDate.text = [NSString stringWithFormat:@"%@",markedDateObj];
        lblSDate.textAlignment = NSTextAlignmentCenter;
        [lblSDate setEnabled:YES];
        NSString *hex = @"0x000000";
        lblSDate.tag=666;
        [lblSDate setTextColor:[UIColor colorWithHexString:hex]];
        lblSDate.font = [UIFont fontWithName:@"MyriadPro-Cond" size:11];
        [self addSubview:lblSDate];
...
-(void)removeImages
{
    ...
    for(UIView *im in views)
    {
        ...
        if([im isMemberOfClass:[UIImageView class]])
        {
        
            [im removeFromSuperview];
        
        }
        if([im isMemberOfClass:[UILabel class]])
        {
...
#pragma mark - Init
-(id)init {
    self = [super initWithFrame:CGRectMake(0, 70, kVRGCalendarViewWidth, 0)];
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont fontWithName:@"MyriadPro-Cond" size:24];
        labelCurrentMonth.textColor = [UIColor colorWithHexString:@"0x1a4d81"];
